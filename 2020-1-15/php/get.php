...
$db = new db();
/*数据库连接语句*/
$db->mysql('localhost', 'root', '', 'anglur', 'leo_', 'utf8');
//print_r($_GET);exit;
...
if($action == 'add_user') {
	//判断用户名存在不存在
	$NAME = $db->rowcount('user', array('username' => "$name"));
    if (!$NAME) {
    	//执行sql插入语句
    	$resultset = $db->insert('user', array('username' => "$name",'password' =>"$password",'addtime'=>time()));
    	echo json_encode(array('code' => 102,'user_id' => $db->lastinsertid()));
    	return;
    }
